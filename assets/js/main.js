...
// Nutze: Math.random(); getElementById; createElement; appendChild; length
...
const test = () => {
    let index = Math.floor(Math.random() * data.length)  //así se elije siempre un elemento random de un array! Y da igual si se eliminan o anaden
    let element = data[index].download_url // así seleccionados sólo 1 elemento del array y sólo a 1 Key del objeto
    console.log(element);
